  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="setup-title">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        {/* <VoiceInputButton onText={(text) => onContextChange(context ? `${context} ${text}` : text)} locale={locale} /> */}
      </div>
      ...
      {(() => {
        const isButtonsValid = complexity.maxButtons >= COMPLEXITY_LIMITS.maxButtons.min && complexity.maxButtons <= COMPLEXITY_LIMITS.maxButtons.max;
        const isWordsValid = complexity.maxWordsPerPhrase >= COMPLEXITY_LIMITS.maxWordsPerPhrase.min && complexity.maxWordsPerPhrase <= COMPLEXITY_LIMITS.maxWordsPerPhrase.max;
        const isFormValid = isButtonsValid && isWordsValid && context.trim().length >= 3;
        ...
        return (
          <button 
            ...
            className="mt-4 w-full rounded-full bg-slate-950 px-6 py-3 text-lg font-black text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
