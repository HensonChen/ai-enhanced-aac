...
function fallbackItems(context: string, maxButtons: number): VocabularyItem[] {
  ...
  return [...base, ...universal].slice(0, maxButtons).map(([word, phrase, category, emoji, isAbstract, type], index) => {
    const cat = category as VocabularyItem["category"];
    return { id: `fallback-${index}-${String(word).replace(/\s+/g, "-")}`, word: String(word), phrase: String(phrase), category: cat, role: inferRoleFromCategory(cat, String(word)), imageUrl: String(emoji ?? FALLBACK_EMOJIS.help), imageSource: "emoji", imageFormat: "png", isAbstract: Boolean(isAbstract), isAnimated: false, type: type as VocabularyItem["type"] };
  });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const items: VocabularyItem[] = generated.items.slice(0, complexitySettings.maxButtons).map((item, index) => ({ id: `${Date.now()}-${index}-${item.word.replace(/\s+/g, "-")}`, ...item, phrase: item.phrase.split(/\s+/).slice(0, complexitySettings.maxWordsPerPhrase).join(" "), role: item.role, imageUrl: "[message]", imageSource: "emoji", imageFormat: "png", isAnimated: false }));
